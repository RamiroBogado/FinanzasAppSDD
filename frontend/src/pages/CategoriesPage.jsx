...
import { createCategory, deleteCategoryCached, getCategories, updateCategory } from '../api.js'
...
const EMPTY_FORM = { name: '', type: 'expense', color: PALETTE[0] }

const TypeToggle = ({ type, onChange }) => (
  <div className="flex gap-2">
    {[
      { value: 'expense', label: 'Gasto', active: 'bg-red-50 text-red-600', inactive: 'bg-white border border-[#E2E8F0]' },
      { value: 'income', label: 'Ingreso', active: 'bg-green-50 text-green-600', inactive: 'bg-white border border-[#E2E8F0]' }
    ].map((option) => (
      <button
        key={option.value}
        type="button"
...
const CategoriesPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await updateCategory(token, editingId, { name, type: form.type, color: form.color })
        toast.showSuccess('Categoría actualizada')
      } else {
        await createCategory(token, { name, type: form.type, color: form.color })
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-[#171d19] dark:text-white">Categorías</h1>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel className="w-full max-w-md rounded-xl border border-[#E2E8F0] bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-4 space-y-4">
              <Field label="Tipo">
                <TypeToggle type={form.type} onChange={(type) => setForm((current) => ({ ...current, type }))} />
              </Field>
              <Field label="Nombre">
                <Input
                  name="name"
                  ...
                  {PALETTE.map((color) => (
                    <button
                      ...
                      className={`flex h-8 w-8 items-center justify-center rounded-full border-2 transition-all ${
                        form.color === color ? 'border-[#0e9f6e]' : 'border-transparent'
                      }`}
